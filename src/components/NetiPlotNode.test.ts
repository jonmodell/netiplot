...
const optsWithSize: NetiPlotOptions = { nodes: { defaultSize: 50 } };

describe('NetiPlotNode constructor', () => {
  it('sets id, x, y from definition', () => {
    const n = new NetiPlotNode('n1', { id: 'n1', x: 10, y: 20 }, opts);
    expect(n.id).toBe('n1');
    expect(n.x).toBe(10);
    expect(n.y).toBe(20);
  });
  ...
  it('converts numeric id to string', () => {
    const n = new NetiPlotNode(42 as unknown as string, { id: '42' }, opts);
    expect(n.id).toBe('42');
  });

  it('defaults x and y to 0 when not provided', () => {
    const n = new NetiPlotNode('n1', { id: 'n1' }, opts);
    expect(n.x).toBe(0);
    expect(n.y).toBe(0);
  });

  it('uses definition.size when provided', () => {
    const n = new NetiPlotNode('n1', { id: 'n1', size: 60 }, opts);
    expect(n.size).toBe(60);
  });

  it('falls back to options.nodes.defaultSize', () => {
    const n = new NetiPlotNode('n1', { id: 'n1' }, optsWithSize);
  ...
  it('sets fixed from definition', () => {
    const n = new NetiPlotNode('n1', { id: 'n1', fixed: true }, opts);
  ...
  it('stores definition reference', () => {
    ...
    const n = new NetiPlotNode('n1', def, opts);
    expect(n.definition).toBe(def);
  });
});

describe('NetiPlotNode update', () => {
  it('updates the definition reference', () => {
    const n = new NetiPlotNode('n1', { id: 'n1', x: 0, y: 0 }, opts);
...
describe('NetiPlotNode render', () => {
  ...
  it('clears destination once arrived', () => {
    const n = new NetiPlotNode('n1', { id: 'n1', x: 99.9, y: 99.9 }, opts);
    n.destination = { x: 100, y: 100 };
    const state = { scale: 1, options: { nodes: {} }, rolloverItem: null };
    n.render(state, ctx, {}, null as unknown as Function);
    expect(n.destination).toBeNull();
